...
// Cargar API Key si ya está en localStorage
if (apiKey) {
...
saveKeyBtn.addEventListener('click', () => {
  ...
  alert('API Key guardada en el navegador.');
  checkReadyToStart();
});
...
function checkReadyToStart() {
  if (apiKey && pdfContext) {
    ...
    statusBadge.textContent = 'Listo para iniciar';
    statusBadge.className = 'px-3 py-1 text-xs rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20';
  }
}

// Inicializar el chat con Google Gen AI
startBtn.addEventListener('click', async () => {
  try {
    const ai = new GoogleGenAI({ apiKey });

    const systemInstruction = `
      You are a professional English recruiter from top tech companies (e.g., GOOGLE, NVIDIA, Apple, Microsoft, Amazon, Zapier, GitLab).
    ...
    alert("Error iniciando la sesión con Gemini: " + err.message);
  }
});
